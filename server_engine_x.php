...
require  'core'.DIRECTORY_SEPARATOR.'server'.DIRECTORY_SEPARATOR.'class.PHPWebSocket.php';
	...
	if ( sizeof($Server->wsClients) == 1 )
		...
		foreach ( $Server->wsClients as $id => $client )
			if ( $id != $clientID ){
                                ...
                                $Server->wsSend($id, json_encode($messageObj));
                        }
}
...
function wsOnClose($clientID, $status) {
	...
	foreach ( $Server->wsClients as $id => $client )
		$Server->wsSend($id, "Visitor $clientID ($ip) has left.");
}
